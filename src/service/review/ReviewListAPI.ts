...
export async function getProductReviews(
    ...
    page: number = 0,
    size: number = 10
): Promise<APIResponse<PageResponse<ReviewResponseItem>>> {
    try {
        const res = await apiClient.get<APIResponse<PageResponse<ReviewResponseItem>>>(
            `/v1/buyers/reviews/${productNumber}/list`,
            { params: { page, size } }
        );
        return res.data;
    } catch (e) {
        return retryIfUnauthorized(e, () => getProductReviews(productNumber, page, size));
    }
}
